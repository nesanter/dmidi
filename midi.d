...
/// Subclass of MidiException generated by a MidiEventBuffer object.
class MidiBufferException : MidiException {
    this(string msg) {
        super(msg);
    }
}
...
//            return new MidiEventBufferMulti(data.header, data.tracks);
    ...
    /+
    /**
     * Set the buffer to ignore certain events based on a filter.
     */
    void push_filter(bool function(MidiEvent ev) filter);
    void pop_filter(); /// ditto
    +/

    /**
     * Skip forward n events. If a filter is given,
     * only count those events that match the filter.
    ...
    override MidiEvent pop_next() {
        while (true) {
            ...
            if (fast_forward)
                ...
                dt = iter.front.delta_time * scale;

            if (dt < index) {
                _offset += dt - base_index;
                base_index = 0;
                index -= dt;

                MidiEvent ev = iter.pop();
